...
A resize action can set `anchor` to control which side of the page appears to move within the fixed GIF canvas. Use `right` to keep the left edge fixed while the right edge moves, `left` to keep the right edge fixed while the left edge moves, or `both` to move both edges equally and keep the page centered. The default is `right`. Use `both` when orientation and symmetric layout changes are easier to understand from a centered page.
