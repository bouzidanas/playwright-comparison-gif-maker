...
For animation, put page preparation that should not appear in the GIF in `scenario.setupActions`. Setup actions replay identically in both versions after the initial route reaches `networkidle`, then focus sampling and synchronization happen before the visible `scenario.actions` timeline begins. Use setup clicks, navigation, input, scrolling, resizing, `waitFor`, and `hold` to reach the intended starting state. Do not rely on `networkidle` to mean that delayed rendering, background data, or animations have settled. Wait for a concrete element or loading state, then add a short hold when the page needs a fixed settling period. Camera `zoom` belongs in visible actions because it is a recording effect, not page setup. Animation must still contain at least one visible action.
...
6. Put hidden preparation and settling in `setupActions`. Include short visible holds before the first recorded interaction and after states a reviewer needs to inspect. Keep the full scenario concise.
...
10. Use short Before and After labels. They are rendered inside the outer top corners of the recordings. The tool appends the short commit SHA to whatever you pass, so never write a SHA yourself. Prefer naming the side and the ref, such as `Before • main` and `After • fix-overflow`, which render as `Before • main (7c52123c)` and `After • fix-overflow (efc0fab4)`. A bare ref name on each side is a good simpler alternative. When the user states a label, pass it exactly as given.
...
Visible actions support `goto`, `click`, `hover`, `fill`, `press`, `scroll`, `resize`, `zoom`, `waitFor`, and `hold`. Setup actions support the same page interactions except `zoom`. Pointer actions accept a Playwright locator string. A resize action requires `width`, `height`, and `resizeMode`, and accepts an optional `durationMs` and `captureStrategy`. A zoom action accepts an optional `locator`, `scale`, and `durationMs`. Add `holdAfterMs` when the resulting state should remain visible.

After the tool returns, a preview panel opens in VS Code with the result, where the user can review the comparison and export the combined, Before, and After artifacts together, so point them to it instead of describing the files alone. Report the GIF path, whether the candidate included uncommitted changes, and the exact baseline and candidate SHAs. Remind the user to regenerate after committing and pushing when the candidate was dirty.